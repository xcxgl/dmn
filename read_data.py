...
def load_babi(data_dir, task_id, type='train'):
    ...
    # Parsing，解析文件的内容，三元组 context/question/answer
    tasks = []  # 三元组的列表
    skip = False  # skip用于判断是否略过这行
    curr_task = None  # 当前解析的三元组
    for i, line in enumerate(open(file_name)):
        id = int(line[0:line.find(' ')])  # 行号
        if id == 1:  # 表示是新的一个三元组，生成新的curr_task
            skip = False
            curr_task = {"C": [], "Q": "", "A": ""}
        ...
        line = line.strip()
        line = line.replace('.', ' . ')  # 处理英文句点
        line = line[line.find(' ') + 1:]
        if line.find('?') == -1: # 文本不包含问号，表示是context
...
if __name__ == '__main__':
    load_babi('data/tasks_1-20_v1-2/en-10k', 1, type='train')
